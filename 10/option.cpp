    ...
    for(int y = 0; y < POINT_NUM; y++){
        for(int x = 0; x < POINT_NUM; x++) {
            ...
            if(!(cloth->points[x][y].bFixed)){
                Vector3d accel( cloth->points[x][y].f.x/Mass*dT,
                                cloth->points[x][y].f.y/Mass*dT,
                                cloth->points[x][y].f.z/Mass*dT);
                //質点の速度
                cloth->points[x][y].v += accel;
                //質点の位置dt*v
                Vector3d vv(cloth->points[x][y].v.x * dT,
                            cloth->points[x][y].v.y * dT,
                            cloth->points[x][y].v.z * dT);
                cloth->points[x][y].p += vv;
                //option追加
                double xc = cloth->points[x][y].p.x - center.x;
                double yc = cloth->points[x][y].p.y - center.y;  
                double zc = cloth->points[x][y].p.z - center.z;
                if(xc*xc + yc*yc + zc*zc < r*r){  //球体の内部に質点が来る。
                    cloth->points[x][y].p -= vv;
